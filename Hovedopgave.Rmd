...
[^bfarrell]
### Custom Element API
Custom Element API er en ny eksperimentel API, som tillader udvikleren at bygge *web components*, som har indbygget understøttelse af alle moderne webbrowsere. Web components tillader udvikleren at lave sine egne brugerdefinerede DOM elementer og bruge dem som komponenter i sin HTML markup. 


### Komponenter

Man kan betragte indbyggede DOM elementer, fx. `input` og `form`, som genbrugelige komponenter, der indkapsler et template (HTML), en style (CSS) og en intern logik (JavaScript). Frameworks som React tillader at lave komponenter [...] På samme måde kan man med den nye `customElements` property på `window` objektet lave genbrugelige komponenter af ren HTML, CSS og JavaScript.
...
CustomElementRegistry interfacet / `customElements` tillader at registrere et hvert objekt, som nedarver fra HTMLElement, som et nyt brugerdefineret element. Når objektet er registreret, kan den bruges som markup direkte i HTML dokumentet. 

Man arbejder med interfacet via `define` metoden på `customElements` property'en på `window` objektet. Hvis man laver en ny klasse, der ligesom alle andre DOM elementer nedarver fra klassen `HTMLElement`, kan den defineres i CustomElementRegistry objektet.
...
Dette vil tillade at bruge `custom-element` tagget i sin HTML markup. For at give `CustomElement` noget funktionalitet, er det nødvendigt at kende til dens *component lifecycle*. Component lifecycle er en række metoder, som Custom Element API der kaldes på forskellige tidspunkter, fx. når elementet tilføjes til DOM eller skal opdateres.

Den første af disse component lifecycle metoder er `connectedCallback`, som tilbydes af det Custom Element API og fyres, når komponenten føjes til DOM.
