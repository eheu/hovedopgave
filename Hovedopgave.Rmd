...
Internettet er et computernetværk bestående af computernetværk som kommunikerer over en fælles protokolstak. Protokolstakken er en gruppe af protokoller til netværkskommunikation, som kan inddeles i lag efter OSI-modellen:
...
kable(protocols_by_layer, caption = "title") %>% kable_styling(latex_options = c("hold_position"), font_size = 14) %>% row_spec(0, font_size = 10)
```
...
JavaScript er det programmeringssprog, som flest udviklere bruger, og har været det siden 2013[^so].
```{R echo = FALSE}
language_by_popularity <- c(JavaScript = 67.8, SQL = 54.5, Java = 41.1, "C#"=31)
barplot(language_by_popularity, main = "Popularitet af programmeringssprog", ylab = "% af adspurgte udviklere", ylim = range(pretty(c(0, language_by_popularity))))
...
# Webpack
create-react-app bruger en automatisk opsat Webpack konfiguration til at kompilere applikationens moduler og afhængigheder til static assets, som er klar til at blive præsenteret for slutbrugeren. Webpack er det, man kalder en *module bundler*[^module-bundler]. Den håndterer altså primært at samle koden ved *import statements* til en samlet JavaScript fil af alle moduler og deres afhængigheder.

Dette giver i sig selv en gevinst, for brugeren kan få hele webapplikationen i et request, og det kan gøre indlæsningen af alle modulerne betydeligt hurtigere på en HTTP/1.1 forbindelse med høj latency.

https://youtu.be/WQue1AN93YU?t=173
```{r echo = FALSE, fig.cap="Hastighed uden bundling"}
  library(DiagrammeR)
graph <- mermaid("
    gantt
...
[^module-bundler]: https://www.npmjs.com/package/react-scripts
