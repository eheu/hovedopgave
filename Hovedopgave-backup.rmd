...
Hvad er et runtime environment
...
Med `require` kan fra et JavaScript modul importere andre JavaScript moduler.
...
CRA bruger en automatisk opsat Webpack konfiguration til at transpilere applikationens kildekode, (hvilket tillader brug af `require` statements)
...
HTTP/2 protokollen tillader asynkron parallel download af static assets, så her gør antallet af requests ikke den store forskel. Udover bundling af JavaScript moduler, kan man med Webpack også håndtere bl.a. importering af stylesheets, billedfiler og JSON, anvendelse af nonstandard syntaks, og meget mere[^webpack].
...
# Micro Frontends
https://dev.to/florianrappl/state-of-microfrontends-1j3p
...
Der er forskel på *autonomous custom elements* (autonome) og *customized built-in elements* (tilpassede). De selvstændige custom elements er helt uafhængige af standard HTML elementer. Man skaber dem ganske enkelt ved at lave et nedarvet objekt fra den generiske `HTMLElement` type. Autonome custom elementer er mere velunderstøttede[^caniuse_customelements] end tilpassede custom elementer. De udvider i stedet funktionaliteten af standard elementerne, så man fx kan lave et brugerdefineret element, der udvider funktionaliteten af `video` tagget fra HTML5, ved at lave et nedarvet objekt fra HTMLVideoElement typen. 
...
Module federation bruger betegnelsen *hosts* for applikationer, der indlæser en *remote*, som er et specielt buildprodukt af Webpack fra det eksterne JavaScript modul. En applikation kan fungere som både host og remote på samme tid. 
...
```js
    ...
    plugins: [
      new ModuleFederationPlugin({
        name: "app_one", // Applikationens navn
        remotes: { // Navn på remotes som skal indlæses
          "app_two": "app_two"
        },
        exposes: { // Interne moduler som skal udstilles som remote
          RemoteNavn: "./sti/til/remote"
        }
      }),
    ],
    // ...
...
Hvis man ønsker at eksponere dele af applikationen som remote, er det for øjeblikket nødvendigt at angive applikationens offentlige webadresse, som den udgives på, i `output` property på `config` objektet til Webpack. Der arbejdes på en løsning[^mf-url-fix], hvor dette kan angives fra konsummenten (hosten) eller udledes automatisk fra importeringen af remote.
...
Når man herefter kører Webpack, vil den som buildprodukt også lave en lille fil ved navn `remoteEntry.js`. Denne fil er host'ens indgangspunkt til remote'en. Den indeholder ikke selve de udstillede moduler, men eksisterer (**HVORFOR?**). Denne fil skal refereres til i host'en, fx ved at indsætte et `<script>` tag i `<head>` tagget af websidens `index.html`:
...
```js
import("app_two/sti/til/modul") // app_two svarer til navnet angivet under remotes
```

```{r echo=FALSE, fig.cap="Deling af afhængigheder mellem microfrontends",dpi=300 ,fig.height=4, fig.width=4 }
library(png)
library(grid)
img <- readPNG("federated-modules-depshare.png")
grid.raster(img)
```

[^mf-url-fix]: https://github.com/webpack/webpack/pull/10703
